...
//			session.setAttribute("userToAdd", user);
			...
			// 確認画面にリダイレクト
			RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/jsp/profileConfirm.jsp");
			dispatcher.forward(req, resp);
...
//			e.printStackTrace();
			...
			ErrorViewData errorData = new ErrorViewData("フォームに入力された内容に問題がありました。",
													"入力画面に戻る","/ActionLoggerSample/profile");
			req.setAttribute("errorData", errorData);
			//エラー表示にフォワード
			RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/jsp/errorVieww.jsp");
			dispatcher.forward(req, resp);
		}
	}
